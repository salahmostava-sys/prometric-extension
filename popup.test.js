...
// ─── Extended Pattern Generator & XLSX Tests ────────────────────────────────
describe('Extended Pattern Variables & XLSX Utilities', () => {
  const { generateCredentials, generateXLSXBlob } = require('./utils.js');

  test('supports {YEAR}, {FIRST}, {first}, {LAST}, {last}, {RAND4}, {RAND2}', () => {
    const currentYear = String(new Date().getFullYear());
    const creds = generateCredentials('Omar Al-Rashidi', '{FIRST}_{last}_{YEAR}_{RAND4}');
    expect(creds).not.toBeNull();
    expect(creds.password.startsWith(`OMAR_alrashidi_${currentYear}_`)).toBe(true);
    // Ensure RAND4 is 4 digits
    const parts = creds.password.split('_');
    expect(parts[3].length).toBe(4);
    expect(/^\d{4}$/.test(parts[3])).toBe(true);
  });

  test('generates valid XLSX blob with ZIP magic numbers', async () => {
    const headers = ['Name', 'Email', 'Status'];
    const rows = [
      ['John Doe', 'john@test.com', 'done'],
      ['Jane Doe', 'jane@test.com', 'failed']
    ];
    const blob = generateXLSXBlob(headers, rows, { sheetName: 'Test' });
    ...
    // PKZip header signature: 0x50, 0x4B, 0x03, 0x04 ('PK\x03\x04')
